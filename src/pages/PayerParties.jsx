...
function PayerParties() {
  ...
  const getData = async () => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_APP_SERVER}/api/sims/get-payer`,
        {
          method: 'GET',
  ...
  const handleAddParty = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch(
        `${import.meta.env.VITE_APP_SERVER}/api/sims/add-payer`,
        {
          method: 'POST',
          headers: {
    ...
    <Box sx={{ p: { xs: 2, md: 3 } }}>
      ...
      >
        <Typography variant='h5' fontWeight={600}>
          List of <span className='text-blue-400'>Payer / Karent Pay</span>{' '}
          Parties
        </Typography>
