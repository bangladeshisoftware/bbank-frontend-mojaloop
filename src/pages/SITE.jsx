...
const P2P = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_APP_SERVER}/api/sims/search-party/MSISDN/${number}/MSISDN/${receiverNumber}`,
        {
          method: 'GET',
      ...
      if (res.ok) {
        ...
        if (
          ress?.party_info?.payer?.status == 200 &&
          ress?.party_info?.payee?.status == 200
        ) {
          ...
          const message =
            ress?.party_info?.payer?.status != 200 &&
            ress?.party_info?.payee?.status != 200
              ? 'Sender and Receiver both Party dose not exits.'
              : ress?.party_info?.payer?.status != 200
                ? 'Sender Party dose not exits!'
                : 'Receiver Party dose not exits!';
          setSearchData({});
          setReceiverData({});
          setStatus('');
  ...
  const handleQuote = async () => {
    ...
    try {
      const res = await fetch(
        `${import.meta.env.VITE_APP_SERVER}/api/sims/quote`,
        {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
                ...
                <div>
                  <div className='mb-2 space-y border border-green-100 rounded-md shadow-sky-50 p-1.5 bg-gray-50'>
                    <Typography variant='p'>
                      Display name: {searchData?.displayName}
                    </Typography>
                    <Typography>First name: {searchData?.firstName}</Typography>
                    <Typography>
                      Middle name: {searchData?.middleName}
                    </Typography>
          ...
          <Box className='p-3 pt-10'>
            ...
            {status == 'view-search-info' ? (
              <div className='mt-2'>
                <div className='mb-2 space-y border border-green-100 rounded-md shadow-sky-50 p-1.5 bg-gray-50'>
                  <Typography variant='p'>
                    Display name: {receiverData?.displayName}
                  </Typography>
